  ...
  return (
    <section id="experience" className="py-20 relative bg-transparent">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Timeline Container */}
        <div className="max-w-4xl mx-auto relative pl-6 sm:pl-8 space-y-10 before:absolute before:left-3 sm:before:left-4 before:top-3 before:bottom-3 before:w-0.5 before:bg-gradient-to-b before:from-cyan-400 before:via-cyan-600/50 before:to-slate-700">
          {filtered.map((item) => (
            <div 
              key={item.id}
            ...
            >
              {/* Timeline Marker Dot */}
              <div className="absolute -left-[21px] sm:-left-[25px] top-2 w-4 h-4 rounded-full bg-cyan-400 ring-4 ring-[#0b0f19] shadow-lg group-hover:scale-125 transition-transform" />

              {/* Card Container */}
              <div className="p-6 sm:p-8 rounded-2xl bg-[#0b0f19] border border-slate-700/80 hover:border-cyan-500/50 shadow-sm transition-all space-y-4">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-800 pb-4">
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      {item.current && (
                        <span className="px-2 py-0.5 rounded bg-[#060810] border border-slate-700 text-cyan-300 text-[10px] font-mono flex items-center gap-1 font-semibold">
                    ...
                    <div className="text-xs font-mono text-cyan-400 uppercase tracking-wider mt-0.5 font-bold">
                      {item.organization}
                    </div>
                  </div>

                  <div className="text-xs font-mono text-slate-300 space-y-1 sm:text-right font-medium">
                  ...
                  <div className="space-y-2 pt-2">
                    ...
                    <ul className="space-y-1.5">
                      {item.achievements.map((ach, i) => (
                        <li key={i} className="text-xs sm:text-sm text-slate-200 flex items-start gap-2 font-normal">
                ...
                {item.technologies && item.technologies.length > 0 && (
                  <div className="pt-2 flex flex-wrap gap-1.5">
                    {item.technologies.map((t) => (
                      <span
                        key={t}
                        className="px-2.5 py-1 rounded bg-[#060810] border border-slate-700 text-slate-300 font-mono text-xs font-medium"
                      >
                        {t}
                      </span>
                    ))}
